...
{
    t_player joueurs[4];
    int nombreJ,i,condition_victoire=0;

    printf("Veuillez saisir le nombre de joeurs : \n");
    scanf("%d",&nombreJ);
...
void pioche(char *jetontire, int *nbtotaljetons)
{
...
/*int main()*/void deroulementtour(t_player joueurj[4],int tourde)
{
    char sens;
    char plateau[15][15];
    char motchoisi[16];
    ...
    //{
        ...
        if (choix==1)
        {
            placementmot(plateau,&sens,motchoisi,&ligne,&colo,&passer/*,&joueurj[tourde]*/);
            lmot=strlen(motchoisi);
            //contester(&contestation);
            verifmot(plateau, motchoisi, ligne, colo, sens,&verif,tour,&joueurj[tourde]);
        }
        if (choix==2)
        {
    ...
    /*joueurj->score=joueurj->score+ / le score structure / scorea*/score(plateau,motchoisi,ligne,colo,sens,&joueurj[tourde]);
    affichermotplace(plateau,motchoisi,ligne,colo,sens);
    ///test
    gotoligcol(16,83);
    //printf("%d",joueurj->score);
    Sleep(2500);
...
void score(char pla[15][15],char motplace[15],int lig, int col,char senspla,t_player *pptjoueur)
{
    ...
    printf("%d",multiplicateur);        ///provisoire
    scoret=scoret*multiplicateur;
    pptjoueur->score=pptjoueur->score+scoret;

    ///provisoire en attendant seb

    gotoligcol(19,83);
    printf("%d",pptjoueur->score);
