...
void deroulementtour()
{
    char sens;
    char motchoisi[16];
    int ligne,colo,i,j,choix,lmot,verif=0,passer=1,quitter=0,tour=0;
    ...
    }while(verif!=lmot+1 || passer!=1 || quitter!=0 || changement!=0);
    tour=tour+1;
}

void placementmot(char tab[15][15],int *psens,char choixlettres, int ligne, int colo,int *pverif,int *ppasser/*,t_player*/ )
{
    do
    {
        //gotoligcol(40,40);
...
void affichermotplace(char place[15][15]/*,motaplacer[15],int lig, int col,char senspla*/)
{
    int lig, col;
    int lmot,k;
    char motaplacer[15];
    ...
    printf("test sasie mot a partir ligne 6 colonne 4 :\n");
    scanf("%s",motaplacer);
    lmot=strlen(motaplacer);
